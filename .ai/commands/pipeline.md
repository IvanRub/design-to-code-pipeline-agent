...
| 2 | [2-gap-analysis](2-gap-analysis.md) | states-analyst → a11y-auditor (sequential) | `gaps.states.json`, `gaps.a11y.json` |
| 3 | [3-spec](3-spec.md) | spec-author | `spec.json` *(frozen)* |
| 4 | [4-generate](4-generate.md) | code-generator | `generated/*`, `generated.meta.json` |
| 5 | [5-validate](5-validate.md) | det. token check → token-validator → qa-reviewer (sequential) | `validation.tokens.json`, `validation.qa.json` |
| 6 | [6-assemble](6-assemble.md) | assembler | `output.json` |

## Control flow
```
1 extract
2 gaps.states && gaps.a11y            (sequential barrier)
3 spec  ── SPEC GATE ───────────────  abort if a required state is missing
repeat (iter = 0 .. 2  — initial run + up to 2 repairs; loop owned by the orchestrator):
   4 generate  (iter>0 ⇒ pass fix_list, regenerate ONLY)
   5 det. token check → token-validator → qa-reviewer   (sequential)
     if token_compliance AND states_coverage numerator == denominator → break
